...
static void vector_iterator_next(iterator_t* it)
{
    ...
    assert(it->position >= vector->data);
    assert(it->position <= vector->data + vector->length * vector->stride);
    
    if(it->position == (uint8_t*)vector->data + vector->length * vector->stride)
        it->position = NULL;
    else
...
static void vector_iterator_prev(iterator_t* it)
{
    ...
    if(it->position == (uint8_t*)vector->data)
        it->position = NULL;
    else
        it->position = (uint8_t*)it->position - vector->stride;
}
...
iterator_t vector_rbegin(const vector_t* vector)
{
    void* ptr = vector->data;
    return iterator(vector_iterator_prev, vector_iterator_next, ptr, (void*)vector);
}

iterator_t vector_rend(const vector_t* vector)
{
    void* ptr = (uint8_t*)vector->data + vector->length * vector->stride;
    return iterator(vector_iterator_prev, vector_iterator_next, ptr, (void*)vector);
}
